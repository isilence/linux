...
const struct bnxt_flow_masks BNXT_FLOW_MASK_NONE = {{{ 0 }}};
...
static int bnxt_alloc_ntp_fltrs(struct bnxt *bp)
{
	...
	bp->ntp_fltr_count = 0;
	bp->ntp_fltr_bmap = kcalloc(BITS_TO_LONGS(BNXT_MAX_FLTR), sizeof(long),
				    GFP_KERNEL);
...
static void bnxt_init_l2_fltr_tbl(struct bnxt *bp)
{
	...
	INIT_LIST_HEAD(&bp->l2_fltr_list);
