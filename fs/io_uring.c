...
{
	u32 seq = ctx->cqs[0].cached_tail - atomic_read(&ctx->cq_timeouts);
...
{
	if (!(req->flags & REQ_F_CQE_SKIP))
		__io_fill_cqe(req->ctx, req->user_data, res, cflags, req->cq_idx);
}
...
{
	ctx->cq_extra++;
	return __io_fill_cqe(ctx, user_data, res, cflags, cq_idx);
}
				 ...
				 unsigned int cflags)
{
	...
	if (!(req->flags & REQ_F_CQE_SKIP))
		__io_fill_cqe(ctx, req->user_data, res, cflags, req->cq_idx);
	/*
...
static int io_iopoll_check(struct io_ring_ctx *ctx, long min)
{
	...
	do {
		...
		if (list_empty(&ctx->iopoll_list)) {
			u32 tail = ctx->cqs[0].cached_tail;
...
static inline bool io_should_wake(struct io_wait_queue *iowq)
{
	...
	int dist = ctx->cqs[0].cached_tail - (int) iowq->cq_tail;
...
static void __io_rsrc_put_work(struct io_rsrc_node *ref_node)
{
	...
	list_for_each_entry_safe(prsrc, tmp, &ref_node->rsrc_list, list) {
		...
		if (prsrc->tag) {
			...
			spin_lock(&ctx->completion_lock);
			io_fill_cqe_aux(ctx, prsrc->tag, 0, 0, IO_DEFAULT_CQ);
			io_commit_cqring(ctx);
			spin_unlock(&ctx->completion_lock);
			io_cqring_ev_posted(ctx);
...
static long io_get_cqring_size(struct io_uring_params *p, u32 entries)
{
	/*
	...
	return roundup_pow_of_two(entries);
}
...
static int io_allocate_scq_urings(struct io_ring_ctx *ctx,
				  struct io_uring_params *p)
{
	u32 __user *cq_sizes = u64_to_user_ptr(p->cq_sizes_ptr);
	struct io_rings *rings;
			   ...
			   struct io_uring_params __user *params)
{
	...
	if (p->nr_cq > IO_MAX_CQS)
		return -EINVAL;
	if (!p->nr_cq != !p->cq_sizes_ptr)
		return -EINVAL;
	...
	if (p->flags & IORING_SETUP_CQSIZE) {
		long nr_entries = io_get_cqring_size(p, p->cq_entries);

		if (nr_entries < 0)
			return nr_entries;
		if (nr_entries < p->sq_entries)
			return -EINVAL;
		p->cq_entries = nr_entries;
	} else {
...
static inline bool io_bpf_need_wake(struct io_kiocb *req)
{
	...
	return __io_cqring_events(&ctx->cqs[bpf->wait_idx]) >= bpf->wait_nr;
}
