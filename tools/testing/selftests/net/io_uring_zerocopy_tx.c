...
static bool cfg_fixed_buf = false;
...
static void do_tx(int domain, int type, int protocol)
{
	...
	do {
		...
		for (i = 0; i < cfg_nr_reqs; i++) {
			...
			io_uring_prep_sendzc(sqe, fd, payload, cfg_payload_len,
					     0, slot_idx, zc_flags);
			if (cfg_fixed_buf) {
		...
		for (i = 0; i < cfg_nr_reqs; i++) {
			...
			if (cqe->user_data == ZC_TAG) {
				...
				/* failed requests don't cfg_flush */
				if (cfg_flush)
					compl_cqes--;
			} else {
...
static void parse_opts(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "46D:p:s:t:n:fb")) != -1) {
		switch (c) {
		case '4':
